...
public class RiskManageAction extends BasePageAction {
	...
	public String queryRiskList() {
		...
		try {
			fundManagementService.queryRiskByCondition(resource, timeStart,
					timeEnd, riskType,userType, pageBean);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
		} 
	...
	public String exportRiskList() {
		...
		try {
			...
			String timeStart = Convert.strToStr(request.getString("timeStart"), null);
			String timeEnd = Convert.strToStr(request.getString("timeEnd"), null);
			String userType = Convert.strToStr(request.getString("userType"), null);
			String riskType = Convert.strToStr(request.getString("riskType"), "");
			...
			fundManagementService.changeNumToStr(pageBean);
			HSSFWorkbook wb = ExcelUtils.exportExcel("风险保障金记录", pageBean
					.getPage(), new String[] { "操作金额", "真实姓名", "企业全称", "用户类型", "操作类型", "详细来源",
					"操作时间" }, new String[] { "amount", "realName", "orgName", "userType", "riskType",
					"resource", "riskDate" });
