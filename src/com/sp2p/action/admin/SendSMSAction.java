...
public class SendSMSAction extends BasePageAction {
	...
	public String querySendSMSListPage() throws Exception {
		...
		try {
			sendSMSService.querySendSMSPage(pageBean, beginTime, endTime,phone);
			List<Map<String, Object>> list = pageBean.getPage();
			if (list != null) {
				for (Map<String, Object> map : list) {
					if (map.get("splitId")!=null) {
	 ...
	 * 重置短信发送次数
	 */
	public String resetSmsAcount() throws SQLException, IOException{
	    String phone = request.getString("phone");
	    long result = sendSMSService.resetSmsAcount(phone);
	    if (result>=0) {
            JSONUtils.printStr("1");
        }else {
            JSONUtils.printStr("2");
