...
public class MyInformationAppAction extends BaseAppAction {
	 ...
	 * 获取省份
	 */
	public String get_province_info() throws IOException{
		Response response = new Response();
		try {
			provinceList = regionService.queryRegionListApp("", 1L, 1,"");
			Map<String, Object> json = new HashMap<String, Object>();
			json.put("province_list", provinceList);
	 ...
	 * 获取市
	 */
	public String get_city_info() throws IOException{
		Response response = new Response();
		try {
			Map<String, String> authAppInfo = getAppInfoMap();
			String province_id = authAppInfo.get("province_id");
			if (StringUtils.isBlank(province_id)) {
				response.failure("参数异常");
				JSONUtils.printObject(response);
				return null;
			}
			provinceList = regionService.queryRegionListApp("", Convert.strToLong(province_id, -1L), 2,"");
			Map<String, Object> json = new HashMap<String, Object>();
			json.put("city_list", provinceList);
			response.success(json);
		} catch (Exception e) {
			e.printStackTrace();
			response.failure("服务器异常");
	 ...
	 * 获取支行
	 */
	public String bank_branch_list() throws IOException{
		Response response = new Response();
		try {
			Map<String, String> authAppInfo = getAppInfoMap();
			String province_id = authAppInfo.get("province_id");
			String city_name = authAppInfo.get("city_name");
			String bank_name = authAppInfo.get("bank_name");
			if (StringUtils.isBlank(province_id) || StringUtils.isBlank(city_name) || StringUtils.isBlank(bank_name)) {
				response.failure("参数异常");
				JSONUtils.printObject(response);
				return null;
			}
			provinceList = regionService.queryRegionListApp(city_name, Convert.strToLong(province_id, -1L), 3,bank_name);
			Map<String, Object> json = new HashMap<String, Object>();
			json.put("bank_branch_list", provinceList);
			response.success(json);
		} catch (Exception e) {
			e.printStackTrace();
			response.failure("服务器异常");
