...
import com.sp2p.database.Dao.Tables.t_transfer_user;
import com.sp2p.database.Dao.Tables.t_trial_borrow;
...
//	}
    ...
    */
    ...
    public List<Map<String, Object>> queryTrialBorrow(Connection conn) throws SQLException, DataException {
        StringBuffer sql = new StringBuffer();
        sql.append("select id from t_trial_borrow where remainTimeStart <= remainTimeEnd and status = 0");
        DataSet dataSet = MySQL.executeQuery(conn, sql.toString());
        dataSet.tables.get(0).rows.genRowsMap();
        sql=null;
        return dataSet.tables.get(0).rows.rowsMap;
	...
	*/
     ...
     * @Throws:
    */
    public Map<String, String> queryMaxTimeTrial(Connection conn, long id,String date) throws SQLException, DataException {
        StringBuffer command = new StringBuffer();
        date=com.shove.web.Utility.filteSqlInfusion(date);
        command.append("select id from t_trial_borrow where status = 0 and remainTimeEnd > '"+StringEscapeUtils.escapeSql(date)+"' and id = "+id);
        DataSet dataSet =MySQL.executeQuery(conn, command.toString());
        command=null;
        return BeanMapUtils.dataSetToMap(dataSet);
    }
	...
	*/
     ...
     * @Throws:
    */
    public Map<String, String> queryMinTimeTrail(Connection conn, long id,String date) throws SQLException, DataException {
        StringBuffer command = new StringBuffer();
        date=com.shove.web.Utility.filteSqlInfusion(date);
        command.append("select publisherName,borrowTitle from t_trial_borrow where status = 0 and remainTimeEnd < '"+ StringEscapeUtils.escapeSql(date)+"'  and id ="+id);
        DataSet dataSet =MySQL.executeQuery(conn, command.toString());
        command=null;
        return BeanMapUtils.dataSetToMap(dataSet);
    }
	...
	*/
     ...
     * @Throws:
    */
    public long updateTimeTrial(Connection conn, long id, String date) throws SQLException {
         date=com.shove.web.Utility.filteSqlInfusion(date);
        String command = "update t_trial_borrow set remainTimeStart = '"+StringEscapeUtils.escapeSql(date)+"' where id ="+id;
        long result=MySQL.executeNonQuery(conn, command);
        command=null;
        return result;
    }
	...
	*/
	...
	/**   
     * @MethodName: updateBorrowTrtialState  
     * @Param: BorrowDao  
     * @Author: gang.lv
     * @Date: 2013-5-4 下午10:58:53
     * @Return:    
     ...
     * @Throws:
    */
    public long updateBorrowTrtialState(Connection conn, long id,int borrowState) throws SQLException {
        String command = "update t_trial_borrow set remainTimeStart = remainTimeEnd,status = "+borrowState+"  where id ="+id;
        long result =  MySQL.executeNonQuery(conn, command);
        command=null;
        return result;
    }
	...
	public Map<String,String> queryBaseFiveApprove(Connection conn, long id,int userType) throws SQLException, DataException{
		
		String command="";
		if(userType==1){
		//个人用户判断
		command = "SELECT userId,SUM(auditStatus) as auditStatus from t_materialsauth  where materAuthTypeId<6 and userId = "+id;
		}else if(userType==2){
		//企业用户判断
		command = "SELECT userId,SUM(auditStatus) as auditStatus from t_materialsauth  where materAuthTypeId<20 and materAuthTypeId>16 and userId = "+id;
		}
		DataSet dataSet =MySQL.executeQuery(conn, command);
		command = null;
	...
	*/
     ...
     * @return
     */
    public List<Map<String, Object>> queryInvesterTrialById(Connection conn,
            long id) throws SQLException, DataException{
        StringBuffer sql = new StringBuffer();
        sql.append("select userId from t_trial_buy t where t.borrowId=  "+id);
        DataSet dataSet = MySQL.executeQuery(conn, sql.toString());
        dataSet.tables.get(0).rows.genRowsMap();
        return dataSet.tables.get(0).rows.rowsMap;
    }
    
    public Map<String, String> queryActivityInvest(Connection conn,String startTime,String endTime,long id) throws DataException{
    	StringBuffer sql = new StringBuffer("SELECT SUM(t.`investAmount`) AS investAmount FROM t_invest t WHERE  t.`deadline`>=3  and t.`investTime`>='").append(startTime).append("' AND t.`investTime` <='").append(endTime).append("' AND t.`investor` = ").append(id);
    	DataSet ds = MySQL.executeQuery(conn, sql.toString());
    	sql = null;
    	return BeanMapUtils.dataSetToMap(ds);
    }
    
    public List<Map<String,String>> queryAwardInfo(Connection conn,String termId){
    	StringBuffer sql = new StringBuffer("SELECT tai.`realMoney`,tap.`picAddr`,tap.`id`,tap.`awardName` FROM t_award_term tat,t_award_info tai,t_award_param tap WHERE tat.`id` = tai.`termId` AND tai.`paramId` = tap.`id` AND tai.`termId` =  ").append(termId);
    	DataSet dataSet = MySQL.executeQuery(conn, sql.toString());
    	sql = null;
    	dataSet.tables.get(0).rows.genRowsMap();
        return dataSet.tables.get(0).rows.rowsMap;
    ...
    public long saveTransfer(Connection conn,String name,String code) throws DataException{
    	String command = "select max(id) as id from t_borrow";
    	DataSet dataSet = MySQL.executeQuery(conn, command);
    	Map<String, String> maxId = BeanMapUtils.dataSetToMap(dataSet);
    	String id = maxId.get("id");
    	Dao.Tables.t_transfer_user transfer_user = new Dao().new Tables().new t_transfer_user();
    	transfer_user._name.setValue(name);
    	transfer_user.borrowId.setValue(id);
    	transfer_user.code.setValue(code);
    	return transfer_user.insert(conn);
    }
}
