...
//	}
	...
	public Map<String, String> queryBorrowMany(Connection conn,
			long borrowId ) throws SQLException, DataException {
		StringBuffer command = new StringBuffer();
		command.append(" SELECT  DISTINCT(count(*)),b.mobilePhone as phone ,(tt.stillInterest+tt.stillPrincipal) as stillPI,a.id as id ,a.feelog as feelog, date_format(a.auditTime,'%Y.%m') as Datesdot ,date_format(a.auditTime,'%Y%m%d')  as DatesNumber ,date_format(a.auditTime,'%Y年%m月%d日')  as DateTime,a.annualRate as annualRate,a.isDayThe as isDayThe,a.deadline as deadline,a.paymentMode as paymentMode ,a.borrowAmount as borrowAmount,s.selectName as purpose,date_format(a.auditTime,'%Y-%m-%d') as starTime,f.idNo as isno,CASE isDayThe WHEN 1 THEN   date_format(DATE_SUB(a.auditTime , INTERVAL -a.deadline MONTH),'%Y-%m-%d')   WHEN 2 THEN date_format(DATE_SUB(a.auditTime , INTERVAL -a.deadline DAY),'%Y-%m-%d')   END  as endTime ,   f_formatting_username(b.username) as Busername,b.username , date_format(a.auditTime ,'%d') as days,a.detail as detail,f.realName as realName,torg.reg_num as reg_num,f.address as address1,torg.address as address2,b.orgName as orgName,b.userType as userType   ");
		command.append(" FROM t_borrow a LEFT JOIN t_user b ON a.publisher = b.id  LEFT JOIN t_orginfo torg ON  b.id = torg.user_id LEFT JOIN t_person f ON f.userId = b.id LEFT JOIN t_select s ON s.selectValue = a.purpose  LEFT JOIN t_repayment  tt on tt.borrowId = a.id  WHERE  a.id ="+borrowId+" and s.typeId =1 " );
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		command = null;
		return BeanMapUtils.dataSetToMap(dataSet);
	}
...
//		return dataSet.tables.get(0).rows.rowsMap;
//	}
	public List<Map<String,Object>>  queryUsername1(Connection  conn, long borrowId ,long invest_id) throws SQLException, DataException{
	    
        StringBuffer command = new StringBuffer();
       command.append(" SELECT u.username as username,toi.address as address2,f.address as address1,u.mobilePhone as phone,f.idNo as idNo,f.realName as realName,u.orgName as orgName,u.userType as userType   FROM   t_repayment a LEFT JOIN t_invest_repayment b  ON a.id = b.repayId    LEFT JOIN t_user u  on u.id = b.`owner`  LEFT JOIN t_orginfo toi on toi.user_id = u.id  LEFT JOIN t_person f ON f.userId =u.id  LEFT JOIN  t_borrow  tb on tb.id = a.borrowId WHERE  a.borrowId = "+borrowId );
       if (invest_id>0) {
           command.append(" and  b.invest_id = "+invest_id );
       }
       command.append("  group by u.username");
	...
	public List<Map<String,Object>>  queryUsername(Connection  conn, long borrowId ,long invest_id) throws SQLException, DataException{
		 ...
		 StringBuffer command = new StringBuffer();
		command.append(" SELECT f_formatting_username(u.username) as username,f.realName as realName,f_formatting_username(u.orgName) as orgName,u.userType as userType   FROM   t_repayment a LEFT JOIN t_invest_repayment b  ON a.id = b.repayId    LEFT JOIN t_user u  on u.id = b.`owner` LEFT JOIN t_person f ON f.userId =u.id  LEFT JOIN  t_borrow  tb on tb.id = a.borrowId WHERE  a.borrowId = "+borrowId );
		if (invest_id>0) {
			command.append(" and  b.invest_id = "+invest_id );
		}
		command.append("  group by u.username");
	...
	*/
	 ...
	 * @throws DataException
	 */
	public Map <String ,String>queryInvestSum(Connection conn, long userId) throws DataException{
		String command = "SELECT SUM(a.recivedPrincipal) investSum FROM t_invest a WHERE a.investor = "+userId;
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		dataSet.tables.get(0).rows.genRowsMap();
		command = null;
		return BeanMapUtils.dataSetToMap(dataSet);
	 ...
	 * @throws DataException
	 */
	public Map <String ,String>queryBorrowSum(Connection conn, long userId) throws DataException{
		String command = "SELECT SUM(borrowAmount) borrowAmount FROM t_borrow a WHERE (a.borrowStatus=4 OR a.borrowStatus=5) and a.publisher = "+userId;
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		dataSet.tables.get(0).rows.genRowsMap();
		command = null;
		return BeanMapUtils.dataSetToMap(dataSet);
	 ...
	 * @throws DataException
	 */
	public Map <String ,String>queryNextRepay(Connection conn, long userId) throws DataException{
		String command = "SELECT DATE_FORMAT(a.repayDate,'%Y-%m-%d') nextRepay FROM t_invest_repayment a WHERE a.repayDate > NOW() AND a.repayStatus=1 and a.owner="+userId+
			" ORDER BY nextRepay LIMIT 1";
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		dataSet.tables.get(0).rows.genRowsMap();
		command = null;
		return BeanMapUtils.dataSetToMap(dataSet);
	 ...
	 * @throws DataException
	 */
	public List<Map <String ,String>> queryNextRepayList(Connection conn, long userId,String date) throws DataException{
		String command = "";
		if (org.apache.commons.lang.StringUtils.isBlank(date)) {
			date = DateUtil.YYYY_MM_DD.format(new Date());
			command = "SELECT DISTINCT DATE_FORMAT(a.repayDate,'%Y-%m-%d') nextRepay FROM t_invest_repayment a WHERE a.repayDate>= DATE_ADD(DATE_ADD(LAST_DAY('"+date+"'),INTERVAL 1 DAY),INTERVAL -1 MONTH) and a.repayDate<=LAST_DAY('"+date+"')  and a.owner="+userId+
					" ORDER BY nextRepay";
		}else {
			/*command = "SELECT DISTINCT DATE_FORMAT(a.repayDate,'%Y-%m-%d') nextRepay FROM t_invest_repayment a WHERE a.repayDate ='"+date+"' and a.owner="+userId+
					" ORDER BY nextRepay";*/
			command = "SELECT DISTINCT DATE_FORMAT(a.repayDate,'%Y-%m-%d') nextRepay FROM t_invest_repayment a WHERE a.repayDate>= DATE_ADD(DATE_ADD(LAST_DAY('"+date+"'),INTERVAL 1 DAY),INTERVAL -1 MONTH) and a.repayDate<=LAST_DAY('"+date+"')  and a.owner="+userId+
					" ORDER BY nextRepay";
		}
	 ...
	 * @throws DataException
	 */
	public List<Map <String ,String>> queryNextRepayDetail(Connection conn, long userId,String date) throws DataException{
		String command = "";
		if (org.apache.commons.lang.StringUtils.isBlank(date)) {
			date = DateUtil.YYYY_MM_DD.format(new Date());
		}
		command = "SELECT tb.`borrowTitle` ,t.`invest_id`,FORMAT((t.`recivedPrincipal`+t.`recivedInterest`),2) money FROM t_invest_repayment t,t_borrow tb WHERE tb.`id` = t.`borrow_id` AND t.repayDate ='"+date+"'  and t.owner="+userId+
				" ORDER BY t.id";
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		dataSet.tables.get(0).rows.genRowsMap();
		command = null;
	 ...
	 * @throws DataException
	 */
	public Map <String ,String> queryNextRepayTotal(Connection conn, long userId,String date) throws DataException{
		String command = "";
		if (org.apache.commons.lang.StringUtils.isBlank(date)) {
			date = DateUtil.YYYY_MM_DD.format(new Date());
			command = "SELECT FORMAT(sum(a.recivedPrincipal+a.recivedInterest),2) total FROM t_invest_repayment a WHERE a.repayDate>= DATE_ADD(DATE_ADD(LAST_DAY('"+date+"'),INTERVAL 1 DAY),INTERVAL -1 MONTH) and a.repayDate<=LAST_DAY('"+date+"')  and a.owner="+userId+
					" ORDER BY a.id";
		}else {
			command = "SELECT FORMAT(sum(a.recivedPrincipal+a.recivedInterest),2) total FROM t_invest_repayment a WHERE a.repayDate ='"+date+"'  and a.owner="+userId+
					" ORDER BY a.id";
		}
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		command = null;
	 ...
	 * @throws DataException
	 */
	public Map <String ,String>queryNextDebt(Connection conn, long userId) throws DataException{
		String command = "SELECT a.repayDate FROM v_t_repayment_detail a WHERE a.publisher= "+userId+
		" AND a.repayDate > NOW() ORDER BY a.repayDate LIMIT 1";
		DataSet dataSet = MySQL.executeQuery(conn, command.toString());
		dataSet.tables.get(0).rows.genRowsMap();
		command = null;
		return BeanMapUtils.dataSetToMap(dataSet);
