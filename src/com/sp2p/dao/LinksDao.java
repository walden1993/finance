...
public class LinksDao {
     ...
     */
    public Long addLinks(Connection conn,String companyName,String companyImg,String companyURL,
            Long serialCount,String dateTime,int type,String description) throws SQLException{
        Dao.Tables.t_links tlinks = new Dao().new Tables().new t_links();
        if(companyName != null)
            tlinks.companyName.setValue(companyName);
        if(companyImg != null)
        ...
        if(dateTime != null)
            tlinks.publishTime.setValue(dateTime);
        tlinks.type.setValue(type);
        tlinks.description.setValue(description);
        return tlinks.insert(conn);
    }
     ...
     */
    public Long addIndexRollImg(Connection conn,String companyImg,
            Long serialCount,String dateTime,String companyURL,Long ordershort,int cardStatus) throws SQLException{
        Dao.Tables.t_links tlinks = new Dao().new Tables().new t_links();
        if(companyImg != null)
            tlinks.companyImg.setValue(companyImg);
        if(serialCount != null && serialCount > 0)
	...
	public Long deleteLinks(Connection conn,String ids) throws SQLException{
		...
		return t_links.delete(conn, "type in (1,5) and id in("+idSQL+")");
	}
	...
	public Long deleteIndexRollImg(Connection conn,String ids) throws SQLException{
		...
		Dao.Tables.t_links t_links = new Dao().new Tables().new t_links();
		return t_links.delete(conn, "type != 1 and id in("+idSQL+")");
	}
	...
	public Map<String, String> queyLinksInfoById(Connection conn, long id) throws SQLException, DataException {
		Dao.Tables.t_links t_links = new Dao().new Tables().new t_links();
		DataSet dataSet = t_links.open(conn, "", "type in(1,5) and id=" + id, "", -1, -1);
	...
	public Map<String, String> queyLinksInfoByIds(Connection conn, long id) throws SQLException, DataException {
		Dao.Tables.t_links t_links = new Dao().new Tables().new t_links();
		DataSet dataSet = t_links.open(conn, "", "type != 1 and id=" + id, "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	/**
	 * 友情链接信息
	 ...
	 * @throws SQLException
	 */
	public Long updateLinks(int id,Connection conn,String companyName,String companyImg,String companyURL,
			Long serialCount,String dateTime) throws SQLException{
		Dao.Tables.t_links t_links = new Dao().new Tables().new t_links();
		if(companyName!=null){
		...
		return t_links.update(conn, "type = 1 and id="+id+ " and type = 1");
	}
    ...
    public Long updateLinks(int id,Connection conn,String companyName,String companyImg,String companyURL,
        ...
        t_links.type.setValue(type);
        t_links.description.setValue(description);
        /*if(publisher!=null){
            questions.publisher.setValue(publisher);
        }
        if(publishTime!=null){
        ...
        return t_links.update(conn, "type in(1,5) and id="+id+ "");
    }
	 ...
	 * @author Administrator
	 */
	public Long updateIndexRollImg(int id,Connection conn,String companyImg,
		...
		return t_links.update(conn, "type != 1 and id="+id + " and type = 3");
	}
    ...
    public Long updateIndexRollImg(int id,Connection conn,String companyImg,
        ...
        return t_links.update(conn, "type != 1 and id="+id + "");
    }
