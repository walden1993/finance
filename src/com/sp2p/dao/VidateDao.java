...
//			tcrediting = new Dao().new Tables().new t_crediting();
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryPerUserCredit2(Connection conn, long id)
			throws SQLException, DataException {
		Dao.Views.v_t_org_credit_msg msg = new Dao().new Views().new v_t_org_credit_msg();
		DataSet dataSet = msg.open(conn, "", " id = " + id, "", -1, -1);
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	...
	public List<Map<String, Object>> queryUserCreditCount(Connection conn,
			...
			String serviceManName, Integer certificateName,String orgName,String userType) throws SQLException, DataException {
		...
		orgName=com.shove.web.Utility.filteSqlInfusion(orgName);
		userType=com.shove.web.Utility.filteSqlInfusion(userType);
	 ...
	 * @throws SQLException
	 */
	public List<Map<String, Object>> queryUserOrgCreditCount(Connection conn,
			String username, String realName, Integer auditStatus,
			String serviceManName, Integer certificateName,String orgName,String userType) throws SQLException, DataException {
		List<Map<String, Object>> lists = null;
		StringBuffer condition = new StringBuffer();
		...
		DataSet ds = vtcm
				.open(
						conn,
						"tmLicenseStatus,tmTaxRegStatus,tmOrgCodeStatus",
						condition.toString(), "", -1, -1);
		ds.tables.get(0).rows.genRowsMap();
		lists = ds.tables.get(0).rows.rowsMap;
		condition = null;
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryOrgTotaPass(Connection conn, Long userId)
			throws SQLException, DataException {
		DataSet dataSet = MySQL
				.executeQuery(
						conn,
						"select COUNT(*) as 'total'  from t_materialsauth tm where tm.userId = "
								+ userId
								+ " AND tm.auditStatus = 3  AND tm.materAuthTypeId > 16 && tm.materAuthTypeId < 20");
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	...
	public Map<String, String> querySelectPassTotal(Connection conn, Long userId)
		...
		DataSet dataSet = MySQL
				.executeQuery(
						...
						"select COUNT(*) as 'total'  from t_materialsauth tm where tm.userId = "
								+ userId
								+ " AND tm.auditStatus = 3  AND tm.materAuthTypeId > 5 && tm.materAuthTypeId <= 16");
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryOrgSelectPassTotal(Connection conn, Long userId)
			throws SQLException, DataException {
		DataSet dataSet = MySQL
				.executeQuery(
						conn,
						"select COUNT(*) as 'total'  from t_materialsauth tm where tm.userId = "
								+ userId
								+ " AND tm.auditStatus = 3  AND tm.materAuthTypeId > 19 && tm.materAuthTypeId <= 34");
		return BeanMapUtils.dataSetToMap(dataSet);
	}
	...
	public Long updateUserPhone(Connection conn, Long userId, String newTelNumber,String userType)
			throws SQLException {
		Dao.Tables.t_user tUser = new Dao().new Tables().new t_user();
		tUser.mobilePhone.setValue(newTelNumber);
		if ("1".equals(userType)) {//个人
            ...
            orginfo.linkman_mobile.setValue(newTelNumber);
            orginfo.update(conn, "  user_id = " + userId);
        }
...
//			creditrating=-creditrating;
		...
		if (creditrating != null && precreditrating != -1 && checkStatus == 3) {
		...
		if (creditrating != null && precreditrating != -1 && checkStatus == 2) {
	...
	public long Updatecreditrating2(Connection conn, Long userId,Integer auditStatus,
			Integer creditrating, Integer mterType) {
		Dao.Tables.t_materialsauth materialsauth = new Dao().new Tables().new t_materialsauth();
