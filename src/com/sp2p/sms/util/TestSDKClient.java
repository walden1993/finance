...
import com.sp2p.sms.Mo;
import com.sp2p.sms.StatusReport;
...
public class TestSDKClient {
	...
	public static void testSendSMS() {
		try {
			int i = SingletonClient.getClient().sendSMS(new String[] { "13192466906" }, "【华融支付】您好，您的验证码是12", "",5);// 带扩展码
	...
	public static void testsSendScheduledSMS() {
		try {
			int i = SingletonClient.getClient().sendScheduledSMSEx(new String[] { "13192466906" },
					"异步内容", "20140828091804", "GBK");
	...
	public static void testsSendSMSAddMessageId() {
		try {
			int i = SingletonClient.getClient().sendSMSEx(new String[] { "15000000000" }, "带有信息ID的短信", "", "GBK", 5, 1234567890123456789L);
			System.out.println("testsSendScheduledSMS=====" + i);
		} catch (Exception e) {
			e.printStackTrace();
		}
