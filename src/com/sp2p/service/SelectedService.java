...
public class SelectedService extends BaseService {
		...
		try {
			...
			for (Map<String, Object> noticeMap : listMap) {
				...
				if ("2".equals(status)) {//已勾选
				    ...
				    if (count==3) {//调用短信发送
                        //如果是还款提醒不需要判断是否开启，其它都要判断
                        if ( "reciveRepayEnable".equals(noticeMode)) {//如果是收到还款则直接发送短信
                            // 调用发短信接口
                            sendMessageService.noteSend(conn, (msgContent
                                    .get("note") + "")
                                    .replaceAll("<br/>", "\n"), userId);
	 ...
	 * Map<String,Object>
	 * @exception 
	 * @date:2014-9-27 下午9:58:51
	 * @since  1.0.0
	 */
	public Map<String, String> getSyspar(String pmcd) throws SQLException{
	    Connection conn = MySQL.getConnection();
	    Map<String,  String> map  = null;
	    try {
            map = selectedDao.getSyspar(conn, pmcd);
        }
        catch (Exception e) {
            e.printStackTrace();
            log.info(e);
