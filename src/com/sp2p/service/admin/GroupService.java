...
public class GroupService extends BaseService {
	 ...
	 * @throws DataException
	 */
	public void queryGroupUserForPage1(String userName, String realName,
			double startAllSum, double endAllSum, double startUseableSum,
			double endUseableSum, long cashStatus, PageBean<Map<String, Object>> pageBean,String investTimes,String investTimee,String startInvestSum,String endInvestSum)
			throws Exception {
		userName = Utility.filteSqlInfusion(userName);
		realName = Utility.filteSqlInfusion(realName);
		...
		if (StringUtils.isNotBlank(investTimes) ) {
			condition.append(" AND investTime >= '");
			condition.append(investTimes).append("'");
		}
		if (StringUtils.isNotBlank(investTimee) ) {
			condition.append(" AND investTime <= '");
			condition.append(investTimee).append("'");
		}
		if (StringUtils.isNotBlank(startInvestSum) ) {
			condition.append(" AND investAmount >= ");
			condition.append(startInvestSum);
		}
		if (StringUtils.isNotBlank(endInvestSum) ) {
			condition.append(" AND investAmount <= ");
			condition.append(endInvestSum);
		}
		try {
			dataPage(conn, pageBean, "v_t_user_person_work", "*", "", condition
					.toString());
