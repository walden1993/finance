...
public class BeVipService extends BaseService {
     ...
     * @throws DataException
     */
    public Map<String, String> queryPersonUser(Long id) throws Exception {
        Connection conn = MySQL.getConnection();

        Map<String, String> map = new HashMap<String, String>();
        try {
            map = beVipDao.queryPersonUser(conn, id);
        } catch (Exception e) {
            log.error(e);
            e.printStackTrace();
	...
	//查询认证过的身份证
	public Map<String, String> queryHasIDCard(String idCard,String realName) throws Exception {
		Connection conn = MySQL.getConnection();
		Map<String, String> map = new HashMap<String, String>();
		try {
			map = beVipDao.queryHasIDCard(conn, StringEscapeUtils
					.escapeSql(idCard),StringEscapeUtils
					.escapeSql(realName));
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
