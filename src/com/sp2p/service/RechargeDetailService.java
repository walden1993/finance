...
public class RechargeDetailService extends BaseService {
	 ...
	 * @throws SQLException 
	 */
	public String payWayQuery(String bankCode) throws SQLException{

		Connection conn = MySQL.getConnection();
		String result = "";
		try {
			result = rechargeDetailDao.payWayQuery(conn, bankCode);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
		} finally {
	...
	/**
	 * 设置订单编号
	 * @param recharId 充值表 ID
	 * @param userId  用户ID 
	 * @return
	 */
	public  String getIn_orderNo(long recharId, long userId){	
		String orderNo = "" ;
		try {
			orderNo =  rechargeDetailDao.getOrderNo(recharId,userId);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return orderNo;
