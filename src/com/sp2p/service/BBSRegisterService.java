...
public class BBSRegisterService {
	 ...
	 */
	public void doRegisterByAsynchronousMode(final Users user) {
	...
	private void doRegister(Users user) throws Exception {
		String password = user.getPassword();
		String bbsuUrl = IConstants.BBS_URL.endsWith("/") ? IConstants.BBS_URL : IConstants.BBS_URL+"/";
		log.info("register bbs URL=" + bbsuUrl);
		String emailConidtion = "";
		//if(user.getEmail().equals("")){
			emailConidtion =user.getMobilePhone()+"@163.com";
		//}else{
		//	emailConidtion = user.getEmail();
		//}
		connect(bbsuUrl,"regsubmit=yes&alipay=&answer=&bday=0000-00-00&bio=&dateformat=0&email="
				+URLEncoder.encode(emailConidtion+"","UTF-8")+"&formHash=6a36c78f&gender=0&icq=&location=&msn=&newsletter=1&password="
				+URLEncoder.encode(password.trim()+"","UTF-8")+"&password2="+URLEncoder.encode(Encrypt.MD5(password.trim()+IConstants.PASS_KEY)+"","UTF-8")+"&pmsound=1&ppp=0&qq=&questionid=0&showemail=1&signature=&site=&styleid=0&taobao=&timeformat=0&timeoffset=9999&tpp=0&user_name="+URLEncoder.encode(user.getUserName()+"","UTF-8")+"&yahoo=&k="+Encrypt.encryptSES(IConstants.BBS_KEY, IConstants.BBS_SES_KEY));
		}
