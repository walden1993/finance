 ...
 * 
 */
public class ValidateService extends CommonService {
	 ...
	 * long
	 * @exception 
	 * @date:2014-12-15 下午3:43:59
	 * @since  1.0.0
	 */
	public long updateBankCardPC(long bankId,long userId,int p,int c) throws SQLException{
	    Connection conn = MySQL.getConnection();
	    long result = -1L;
	    try {
            Dao.Tables.t_bankcard tBankcard = new Dao().new Tables().new t_bankcard();
            tBankcard.province.setValue(p);
            tBankcard.city.setValue(c);
            result = tBankcard.update(conn, " id = "+bankId+" and userId = "+userId+" and isDelete = 2");
            if (result<0) {
                conn.rollback();
            }
            conn.commit();
	 ...
	 * @throws SQLException 
	 * @exception 
	 * @date:2014-12-15 下午2:50:25
	 * @since  1.0.0
	 */
	public Map<String,String> queryBankCard(long id,long userId) throws SQLException{
	    Connection conn = MySQL.getConnection();
	    try {
            DataSet ds = Database.executeQuery(conn, "SELECT * FROM t_bankcard t WHERE t.`id` = "+id+" AND t.`userId` = "+userId);
	 ...
	 * @param autiStatus
	 *            状态值
	 * @param certificateName 审核状态
	 * @param serviceManName 审核人名称
	 * @param orgName 企业名称
	 * @param userType 用户类型
	 * @throws SQLException
	 */
	public void queryPersonInfo(PageBean<Map<String, Object>> pageBean,
			...
			Integer certificateName, String serviceManName,String orgName,String userType) throws Exception {
		...
		if (StringUtils.isNotBlank(orgName)) {
			condition.append(" AND orgName  LIKE CONCAT('%','"
		...
		if (null != auditStatus && auditStatus > -1 && auditStatus == 1// 工作信息
		...
		if (null != auditStatus && auditStatus > -1 && auditStatus == 2// 基本信息
				&& null != certificateName && certificateName > -1) {
			if (certificateName != 4) {
		...
		if (null != auditStatus && auditStatus > -1 && auditStatus == 3// 联系信息
				&& null != certificateName && certificateName > -1) {
			if (certificateName != 4) {
		...
		// ------------
		if (auditStatus <= -1 && certificateName > -1 && certificateName != 4) {
			condition.append(" AND ( workauditStatus = " + certificateName
		...
		if (auditStatus <= -1 && certificateName == 4) {
			condition.append(" AND directedStatus is null");
		}
		// --------------
		try {
	...
	public void querycreditLimitApply(PageBean<Map<String, Object>> pageBean,
			...
			String endTime,String userType) throws Exception {
		username = Utility.filteSqlInfusion(username);
		endTime = Utility.filteSqlInfusion(endTime);
		userType = Utility.filteSqlInfusion(userType);
		StringBuffer condition = new StringBuffer();
		System.out.println("---------------------------aa:"+userType);
		if (StringUtils.isNotBlank(username)) {
	...
	public void queryUserCredit(PageBean<Map<String, Object>> pageBean,
			String username, String realName, Integer auditStatus,
			String serviceManName, Integer certificateName,String orgName,String userType) throws Exception {
		username = Utility.filteSqlInfusion(username);
		realName = Utility.filteSqlInfusion(realName);
		serviceManName = Utility.filteSqlInfusion(serviceManName);
		orgName = Utility.filteSqlInfusion(orgName);
		...
		if (null != auditStatus && auditStatus > -1) {
			...
			if (certificateName != null && certificateName > -1
					&& certificateName == 6) {
				condition.append(" AND tmLicenseStatus = " + auditStatus);
			}
			// ============================
			if (certificateName != null && certificateName > -1
					&& certificateName == 7) {
				condition.append(" AND tmTaxRegStatus = " + auditStatus);
			}
			// ============================
			if (certificateName != null && certificateName > -1
					&& certificateName == 8) {
				condition.append(" AND tmOrgCodeStatus = " + auditStatus);
			}
			// ===========================
	 ...
	 * @throws SQLException
	 */
	public void queryUserOrgCredit(PageBean<Map<String, Object>> pageBean,
			String username, String realName, Integer auditStatus,
			String serviceManName, Integer certificateName,String orgName,String userType) throws Exception {
		username = Utility.filteSqlInfusion(username);
		realName = Utility.filteSqlInfusion(realName);
		...
		if (null != auditStatus && auditStatus > -1) {
			...
			if (certificateName == -1) {
				condition.append(" and (tmLicenseStatus = " + auditStatus 
						+ " or tmTaxRegStatus = " + auditStatus 
						+ " or tmOrgCodeStatus = " + auditStatus + ") ");
			}
		}
	...
	public List<Map<String, Object>> queryUserCreditCount(String username,
			...
			Integer certificateName,String orgName,String userType) throws Exception {
		List<Map<String, Object>> map = new ArrayList<Map<String, Object>>();
		Connection conn = MySQL.getConnection();
		try {
			map = vidateDao.queryUserCreditCount(conn, username, realName,
					auditStatus, serviceManName, certificateName,orgName,userType);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
		} finally {
	 ...
	 * @throws DataException
	 */
	public List<Map<String, Object>> queryUserOrgCreditCount(String username,
			String realName, Integer auditStatus, String serviceManName,
			Integer certificateName,String orgName,String userType) throws Exception {
		List<Map<String, Object>> map = new ArrayList<Map<String, Object>>();
		Connection conn = MySQL.getConnection();
		try {
			map = vidateDao.queryUserOrgCreditCount(conn, username, realName,
					auditStatus, serviceManName, certificateName,orgName,userType);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public Map<String, String> queryPerUserCredit2(Long userId) throws Exception {
		Map<String, String> map = null;
		StringBuffer condition = new StringBuffer();
		if (userId != null && userId >= -1) {
			condition.append(" AND id = " + userId);
		...
		Connection conn = MySQL.getConnection();
		try {
			map = vidateDao.queryPerUserCredit2(conn, userId);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
		} finally {
	 ...
	 * @throws SQLException
	 */
	public Map<String, String> queryOrgTotaPass(Long userId) throws Exception {
		Map<String, String> map = null;
		Connection conn = MySQL.getConnection();
		try {
			map = vidateDao.queryOrgTotaPass(conn, userId);
	...
	public void querytelphonePage(Long userId, PageBean pageBean,
			String username, String starttime, String endtime,String userType) throws Exception {
		username = Utility.filteSqlInfusion(username);
	...
	public void querytelphonePage2(Long userId, PageBean pageBean,
			String username, String starttime, String endtime, Integer statuss,String userType)
			throws Exception {
		username = Utility.filteSqlInfusion(username);
		starttime = Utility.filteSqlInfusion(starttime);
		endtime = Utility.filteSqlInfusion(endtime);
		...
		sqlresult
				.append(" tpbi.id as tpbiid, tuser.id as id, tuser.username as username, a.co as amountall,tp.realName as realName,tuser.userType as userType, torg.organization_name as orgName, ");
		sqlresult
		...
		sql.append(" left join  t_user tuser   on tuser.id = tpbi.userId ");
		sql.append(" left join t_person tp on tuser.id = tp.userId ");
		sql.append(" left join t_orginfo torg on tuser.id = torg.user_id ");
		sql.append(" left join ");
		sql
	 ...
	 * @throws SQLException
	 */
	public Map<String, String> queryOrgSelectPassTotal(Long userId)
			throws Exception {
		Map<String, String> map = null;
		Connection conn = MySQL.getConnection();
		try {
			map = vidateDao.queryOrgSelectPassTotal(conn, userId);
	...
	public List<Map<String, Object>> querySelectPictureDate(
		...
		StringBuffer sqlresult = new StringBuffer();
		sqlresult
				.append(" vtb.tmid  as tmid,tmd.imagePath as imgPath,tmd.id as tmdid,tmd.`option` as tmoption,vtb.username as username,vtb.id as id,vtb.realName as realName,vtb.checkperson as checkperson,vtb.tmoption as alltmoption,vtb.orgName as orgName,vtb.userType as userType,tmd.uploadingTime as passTime,tmd.checktime as checktime,vtb.materAuthTypeId as materAuthTypeId,tmd.auditStatus as auditStatus,vtb.tmtname as tmtname ");
		System.out.println(sqlresult);
		StringBuffer sql = new StringBuffer();
		sql.append(" t_materialimagedetal tmd ");
		sql
	...
	public List<Map<String, Object>> queryPictureDateCount(
			PageBean<Map<String, Object>> pageBean, Long userId,
			Integer materAuthTypeId, String username, String realName,String orgName,String userType)
			throws Exception {
		username = Utility.filteSqlInfusion(username);
		realName = Utility.filteSqlInfusion(realName);
	...
	public List<Map<String, Object>> queryselect(
			...
			Integer typeStatus,String orgName,String userType) throws Exception {
		username = Utility.filteSqlInfusion(username);
		realName = Utility.filteSqlInfusion(realName);
		...
		if (StringUtils.isNotBlank(orgName)) {
			condition.append(" and torg.organization_name  like '%"
					+ StringEscapeUtils.escapeSql(orgName.trim()) + "%' ");
		}
		if (StringUtils.isNotBlank(userType) && !"-1".equals(userType)) {
			condition.append(" and tuser.userType ='"
					+ StringEscapeUtils.escapeSql(userType.trim()) + "'");
		}
		...
		sqlresult.append(" tuser.userType as userType, ");
		sqlresult.append(" torg.organization_name as orgName, ");
		...
		sql
				.append(" left join (select tuser.id,count(*) as wait,tuser.username from t_materialsauth tm , t_user tuser where tm.auditStatus = 1 AND tuser.id = tm.userId  AND tm.materAuthTypeId NOT IN(1,2,3,4,5,17,18,19) GROUP BY tuser.username) w on tuser.id = w.id  ");
		sql
				.append(" left join (select tuser.id,count(*) as fail,tuser.username from t_materialsauth tm , t_user tuser where tm.auditStatus = 2 AND tuser.id = tm.userId AND tm.materAuthTypeId NOT IN(1,2,3,4,5,17,18,19) GROUP BY tuser.username) f on tuser.id = f.id   ");
		sql
				.append(" left join (select tuser.id,count(*) as success,tuser.username from t_materialsauth tm , t_user tuser where tm.auditStatus = 3 AND tuser.id = tm.userId AND tm.materAuthTypeId NOT IN(1,2,3,4,5,17,18,19) GROUP BY tuser.username ) s on tuser.id = s.id  ");
	...
	public List<Map<String, Object>> queryselect1(Integer materAuthTypeId,
		...
		selectSelect
				.append(" tuser.id as id,tuser.userType as userType,tuser.username  as username,torg.organization_name as orgName,tp.realName as realName,IFNULL(temp1.noshangchuan,0) as nosh,IFNULL(w.wait,0) as wait, ");
		selectSelect
		...
		selectSelect.append(" left join t_orginfo torg on tuser.id = torg.user_id ");
		selectSelect
				.append(" left join v_t_noshangchuan  temp1 on tuser.id = temp1.id ");
		selectSelect
				.append(" left join (select tuser.id,count(*) as wait,tuser.username from t_materialsauth tm , t_user tuser where tm.auditStatus = 1 AND tuser.id = tm.userId  AND tm.materAuthTypeId NOT IN(1,2,3,4,5,17,18,19) GROUP BY tuser.username) w on tuser.id = w.id  ");
		selectSelect
				.append(" left join (select tuser.id,count(*) as fail,tuser.username from t_materialsauth tm , t_user tuser where tm.auditStatus = 2 AND tuser.id = tm.userId AND tm.materAuthTypeId NOT IN(1,2,3,4,5,17,18,19) GROUP BY tuser.username) f on tuser.id = f.id  ");
		selectSelect
				.append(" left join (select tuser.id,count(*) as success,tuser.username from t_materialsauth tm , t_user tuser where tm.auditStatus = 3 AND tuser.id = tm.userId AND tm.materAuthTypeId >5 AND tm.materAuthTypeId NOT IN(1,2,3,4,5,17,18,19) GROUP BY tuser.username ) s on tuser.id = s.id  ");
		...
		if (StringUtils.isNotBlank(orgName)) {
			selectSelect.append(" and organization_name  like '%"
					+ StringEscapeUtils.escapeSql(orgName.trim()) + "%' ");
		}
		if (StringUtils.isNotBlank(userType) && !"-1".equals(userType)) {
			selectSelect.append(" and tuser.userType ='"
					+ StringEscapeUtils.escapeSql(userType.trim()) + "'");
		}
		if (typeStatus != null && typeStatus != -1) {
			if (typeStatus == 1) {
	...
	public void querynewUserCheck(PageBean<Map<String, Object>> pageBean,
			String username,String userType) throws Exception {
		username = Utility.filteSqlInfusion(username);
		userType = Utility.filteSqlInfusion(userType);
	 ...
	 * @throws SQLException
	 */
	public Long updateUserPhoneService(String userType,Long userId, Integer auditStatus,
		...
		try {
			...
			{
				resultId = vidateDao
						.updateUserPhone(conn, userId, newTelNumber,userType);
				if (resultId <= 0) {
					conn.rollback();
					return -1L;
				}
	...
	public Long updateUserCreditLimit(Long userId, Integer Creditstatus,
		...
		Long resultId = -1L;
		try {
		    if (Creditstatus==2) {//审核不通过
		        Map<String, String> map = queryrequestCredit(ti);
		        if (map!=null && !"".equals( map.get("username"))) {
		            Integer oldStatus = Convert.strToInt(map.get("tcstatus"), -1);//原始的审核状态
	                if (oldStatus>1) {//表示这条记录已经被审核过了
	                    BigDecimal oldApplyAmount  = new BigDecimal(map.get("agreeAmount"));
	                    vidateDao.updateUserCreditLimit(conn, userId,
	                                                    Creditstatus, oldApplyAmount.negate(), checkMsg, adminId, ti);//减去原来增加的信用额度
	                }
                }
            }
