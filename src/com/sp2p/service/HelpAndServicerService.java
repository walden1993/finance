...
public class HelpAndServicerService extends BaseService {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void queryCallcenterSearch(String order, PageBean pageBean, String queryString)
			throws Exception {
		order = Utility.filteSqlInfusion(order);
		
		Connection conn = MySQL.getConnection();
		try {
			dataPage(conn, pageBean, "v_t_callcenter_help_list v", " * ", "",
					" and v.`typeDescribe` LIKE '%"+queryString+"%' OR v.`question` LIKE '%"+queryString+"%' OR v.`anwer`  LIKE '%"+queryString+"%'");
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	 ...
	 * @throws DataException
	 */
	public Map<String, String> queryAnswer(long Id) throws Exception {
		Connection conn = MySQL.getConnection();
		Map<String, String> map = null;
		try {
			map = callCenterDao.queryAnswerById(conn, Id);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	 ...
	 * @throws DataException
	 */
	public List<Map<String,Object>> queryAnswerByTypeId(long typeId) throws Exception {
		Connection conn = MySQL.getConnection();
		List<Map<String,Object>> map = null;
		try {
			map = callCenterDao.queryAnswerByTypeId(conn, typeId);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
