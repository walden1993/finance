...
public class PublicModelService extends BaseService {
	...
	/**
     * 添加友情链接 2014-10-21 09:19:28  hjh
     */
    public Long addLinks(String companyName, String companyImg,
            String companyURL, long serialCount, String dateTime,int type,String description)
            throws Exception {
        Connection conn = MySQL.getConnection();
        Long commonId = -1L;
        try {
	...
	public void queryLinksPage(PageBean<Map<String, Object>> pageBean)
		...
		Connection conn = MySQL.getConnection();
		try {
			dataPage(conn, pageBean, "t_links", "*", " order by serialCount asc", "   and type in (1) ");
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
			throw e;
	 ...
	 * @throws SQLException
	 */
	public void queryLinksListPageBack(PageBean<Map<String, Object>> pageBean)
			throws Exception {
		Connection conn = MySQL.getConnection();
		try {
			dataPage(conn, pageBean, "t_links", "*", " order by serialCount asc", "   and type in (1,5) ");
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
			throw e;
     ...
     * @throws SQLException
     */
    public void queryAppImgPage(PageBean pageBean)
            throws Exception {
        Connection conn = MySQL.getConnection();
        try {
            dataPage(conn, pageBean, " t_links ", "companyImg as banner_img_ulr,companyURL as banner_href,ordershort as sort_id",
                    "  order by ordershort asc ", " and type = 4 ");
        } catch (Exception e) {
            log.error(e);
            e.printStackTrace();
            throw e;
	 ...
	 * @throws SQLException
	 */
	public Long updateLinks(int id,String companyName, String companyImg,
			String companyURL, long serialCount, String dateTime)
			throws Exception {
		Connection conn = MySQL.getConnection();
		try {
			linksDao.updateLinks(id,conn, companyName, companyImg, companyURL,
    ...
    public Long updateLinks(int id,String companyName, String companyImg,
        ...
        try {
            linksDao.updateLinks(id,conn, companyName, companyImg, companyURL,
                    serialCount, dateTime,type,description);
            conn.commit();
        } catch (Exception e) {
            log.error(e);
            e.printStackTrace();
	...
	public Long addIndexRollImg(String companyImg, long serialCount,
			String dateTime, long ordershort, int cardStatus)
			throws Exception {
		Connection conn = MySQL.getConnection();
		Long commonId = -1L;
		Long newsId = -1L;
		try {
			commonId = linksDao.addIndexRollImg(conn, companyImg, serialCount,
					dateTime, ordershort, cardStatus);
			if (commonId < 0) {// 如果小于0，添加失败，回滚
				conn.rollback();
			}
			conn.commit();
     ...
     */
    public Long addIndexRollImg(String companyImg, long serialCount,
            String dateTime,String companyURL, long ordershort, int cardStatus)
            throws Exception {
        Connection conn = MySQL.getConnection();
        Long commonId = -1L;
        Long newsId = -1L;
	 ...
	 * @author Administrator
	 */
	public Long updateIndexRollImg(int id,String companyImg, long serialCount,
			String dateTime, long ordershort) throws Exception {
		Connection conn = MySQL.getConnection();
		try {
			linksDao.updateIndexRollImg( id,conn, companyImg, serialCount,
					ordershort, dateTime);
			conn.commit();
		} catch (Exception e) {
			log.error(e);
     ...
     */
    public Long updateIndexRollImg(int id,String companyImg, long serialCount,
            String dateTime,String companyURL, long ordershort) throws Exception {
        Connection conn = MySQL.getConnection();
        try {
            linksDao.updateIndexRollImg(id,conn, companyImg, serialCount,
	...
	/**
	 * 查询推荐标，如果没有推荐，就查询最近一期的标
	 */
	public Map<String, String> queryRecommandBorrow() throws SQLException{
	    Connection conn = MySQL.getConnection();
	    Map<String, String> map = new HashMap<String, String>();
	    try {
            DataSet ds =  Database.executeQuery(conn, "SELECT t.borrowStatus,t.borrowTitle,t.deadline,t.id,t.isDayThe,t.amount_scale,t.annualRate,t.hasInvestAmount,t.borrowAmount FROM t_borrow t WHERE t.stickie=0  and  t.borrowStatus>=2 and  t.borrowStatus<=5    ORDER BY  t.borrowStatus asc,  t.id DESC LIMIT 0,1");
            map = BeanMapUtils.dataSetToMap(ds);
            if (map==null || map.get("borrowTitle")==null || "".equals(map.get("borrowTitle"))) {
                ds = Database.executeQuery(conn, "SELECT t.borrowStatus,t.borrowTitle,t.id,t.deadline,t.isDayThe,t.amount_scale,t.annualRate,t.hasInvestAmount,t.borrowAmount FROM t_borrow t where  t.borrowStatus>=2 and  t.borrowStatus<=5   ORDER BY  t.borrowStatus asc,  t.id DESC  LIMIT 0,1");
                map = BeanMapUtils.dataSetToMap(ds);
            }
        }
        catch (Exception e) {
	 ...
	 * @throws SQLException 
	 * @exception 
	 * @date:2014-11-3 下午5:44:59
	 * @since  1.0.0
	 */
	public Map<String, String>  modelValue(long borrowId) throws SQLException{
	     Map<String, String>  map = new HashMap<String, String>();
	     String sql_db = "SELECT t.id,t.publisher,ts.`selectName` FROM t_borrow t  LEFT JOIN t_select ts  on ts.`id` = t.`agent` and ts.`typeId` =2  WHERE t.`id`="+borrowId;//查询担保
	     Connection conn = MySQL.getConnection();
	     try {
            DataSet ds = Database.executeQuery(conn, sql_db);
            Map<String, String>  db = BeanMapUtils.dataSetToMap(ds);
            if (db==null || "".equals(db.get("publisher"))) {
                return null;
            }
            long userId = Convert.strToLong(db.get("publisher"), -1);
            String sql_bank="SELECT b.`cardUserName`,b.`bankName`,b.`cardNo` FROM t_bankcard b  WHERE b.`userId`= "+userId;
            ds = Database.executeQuery(conn, sql_bank);
            Map<String, String>  bk = BeanMapUtils.dataSetToMap(ds);
            if (bk==null || "".equals(db.get("cardUserName"))) {
                bk = new HashMap<String, String>();
                bk.put("cardUserName", "无");
                bk.put("cardNo", "无");
                bk.put("bankName", "无");
            }
