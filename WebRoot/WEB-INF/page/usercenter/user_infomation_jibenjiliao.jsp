  ...
  $('#cellPhone1').html(tmplen1+' **** '+tmplen2); 
	...
	<body>
		...
		<div class="nymain wrap">
			<div class="bigbox w950" style="border:none">
								...
								<div class="tab">
									<form id="BaseDataform">
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
											<s:if test="#request.map.realName!=null && #request.map.realName!=''">
											<tr>
                                                <td align="right">
                ...
                <s:else>
                        <tr>
                    <td align="right" class="red f14">
                        <span style="color: red; float: none;" class="formtips"  >*</span>提示：
                    </td>
                    <td>
                       <p  class="red f14"    id="isAuth"> 你尚未进行实名认证，<a href="securityCent.mht?isOn=3" class="blue">立即认证</a></p>
                    </td>
                </tr>
                </s:else>
		        ...
		        $("#u_cellPhone").html("请填写手机号码！");
	                  ...
	                  $("#u_cellPhone").html("请正确填写手机号码！");
                  	  ...
                  	  var cellPhone=$("#cellPhone").val();
                   		$.post("cellphoneOnly.mht","cellphone="+cellPhone,function(data){
                   			if(data.msg=="手机已存在"){
								 $("#u_cellPhone").attr("class", "formtips onError");
	                        	...
	                        	$("#u_cellPhone").html(""); 
	                       		if($("#clickCode").html()!="重新发送验证码"||$("#clickCode").html()!="点击获取验证码") { 
	                       			//当处在发送中的状态时候
	                      	  		...
	                      	  		flags = false;
		                      		$.post("removeCode.mht","",function(data){});//删除手机验证码
		                		 }
	                   		}
	                    });
...
$(document).ready(function(){
  ...
  if($(this).attr("id")=="idNo"){
    ...
    if($(this).val() ==""){
      ...
      $.post("isIDNO.mht",parama,function(data){
       if(data.putStr=="0"){
        $("#u_idNo").html("请填写身份证号码");
 ...
 //============
	...
	//提交基础资料
	  $("#jc_btn").click(function(){
		  ...
		  var isAuth = $("#isAuth").html();
          if(isAuth!=undefined){
              if(confirm("你尚未进行实名认证，请实名认证后在操作")){
            	  window.location.href="securityCent.mht?isOn=3"
              }
              return false;
          }
...
//	    }
	    ...
	    */
	    ...
	    var isCellPhone = '${map.cellPhone}';
        if(isCellPhone!=null && isCellPhone!=''){
            param["paramMap.isCellPhone"]=isCellPhone;
        }
        
	    $.post("updateBasedate.mht",param,function(data){
	       if(data.msg=="保存成功"){
	         window.clearInterval(tipId);
	         $("#clickCode").html("点击获取验证码");
	         var from = '${from}';
	         ...
	         //如果不是从发布借款跳转过来，那么按照正常的步骤一步一步走
	        	window.location.href='querWorkData.mht';
 ...
 function getData(){
   ...
   var sexNum = "";
   if (idv.length == 18){
	  bd.val((idv).substr(6,4)+'-'+(idv).substr(10,2)+'-'+(idv).substr(12,2));
	  sexNum = (idv).substr(16,1);
	  var param1=$("input[name='paramMap_sex']");
	  if (parseInt(sexNum)%2 == 0){
		   param1[1].checked = true;
	   } else {
		   param1[0].checked = true;
	   }
   }
   
   //bd.value=(idv).substr(0,4)+'-'+(idv).substr(4,2)+'-'+(idv).substr(6,2);
