  ...
  <div class="sqdk" style="background:none;height:auto;padding: 0px;">
    ...
    <div class="tcbox" style="display:-none; position:static; margin:0px auto; padding:0px;">
      ...
      <s:if test="#request.typmap.materAuthTypeId==6">
            <h3>房产认证：</h3>
      <p>房产证明是证明借入者资产及还款能力的重要凭证,三好资本会根据借款者提供的房产证明给与借入者一定的信用加分。</p>
...
<br />
      ...
      <s:if test="#request.typmap.materAuthTypeId==2">
            <h3>工作认证：</h3>
      <p>您的工作状况是三好资本评估您信用状况的主要依据。请您填写真实可靠的工作信息。 </p>
      <p><strong>认证说明：</strong><br />
...
3、 请确认您上传的资料是清晰的、未经修改的照片，每张照片最大限制为1M
      ...
      <s:if test="#request.typmap.materAuthTypeId==3">
            <h3>居住地认证：</h3>
      <p>居住地的稳定性，是三好资本考核借款人的主要评估因素之一，通过三好资本居住地证明，您将获得一定的信用加分。 </p>
      <p><strong>认证说明：</strong><br />
...
2、 请确认您上传的资料是清晰的、未经修改的照片，每张照片最大限制为1M
      ...
      <s:if test="#request.typmap.materAuthTypeId==4">
            <h3>央行信用认证：</h3>
      <p>个人信用报告是由中国人民银行出具，全面记录个人信用活动，反映个人信用基本状况的文件。本报告是三好资本了解您信用状况的一个重要参考资料。 您信用报告内体现的信用记录，和信用卡额度等数据，将在您发布借款时经三好资本工作人员整理，在充分保护您隐私的前提下披露给三好资本借出者，作为借出者投标的依据。</p>
      <p><strong>认证说明：</strong><br />
...
3、 请确认您上传的资料是清晰的、未经修改的照片，每张照片最大限制为1M
      ...
      <s:if test="#request.typmap.materAuthTypeId==5">
            <h3>收入认证：</h3>
      <p>您的银行流水单以及完税证明，是证明您收入情况的主要文件，也是三好资本评估您还款能力的主要依据之一。 </p>
      <p><strong>认证说明：</strong><br />
      1、工薪阶层（入职须6个月以上）：<br />
请上传以下一项或者多项资料的照片<br />
...
3、 请确认您上传的资料是清晰的、未经修改的照片，每张照片最大限制为1M
      ...
      <s:if test="#request.typmap.materAuthTypeId==7">
            <h3>车辆认证：</h3>
      <p>购车证明是证明借入者资产及还款能力的重要凭证之一，三好资本会根据借入者提供的购车证明给与借入者一定的信用加分。</p>
      <p><strong>认证说明：</strong><br />
...
3、 请确认您上传的资料是清晰的、未经修改的照片，每张照片最大限制为1M<br />
      ...
      <s:if test="#request.typmap.materAuthTypeId==8">
            <h3>结婚认证：</h3>
      <p>借入者的婚姻状况的稳定性，是三好资本考核借款人信用的评估因素之一，通过三好资本结婚认证，您将获得一定的信用加分。</p>
      <p><strong>认证说明：</strong><br />
 1、	请您上传以下资料<br />
...
2、 请确认您上传的资料是清晰的、未经修改的照片，每张照片最大限制为1M
      ...
      <s:if test="#request.typmap.materAuthTypeId==10">
            <h3>技术职称认证：</h3>
      <p>技术职称是经专家评审、反映一个人专业技术水平并作为聘任专业技术职务依据的一种资格，不与工资挂钩，是三好资本考核借款人信用的评估因素之一，通过三好资本技术职称认证证明，您将获得一定的信用加分。</p>
      <p><strong>认证说明：</strong><br />
...
2、 请确认您上传的资料是清晰的、未经修改的照片，每张照片最大限制为1M
      ...
      <s:if test="#request.typmap.materAuthTypeId==11">
            <h3>手机认证：</h3>
      <p>手机流水单是最近一段时间内的详细通话记录，是三好资本用以验证借入者真实性的重要凭证之一。您的手机详单不会以任何形式被泄露。</p>
      <p><strong>认证说明：</strong><br />
  ...
  <tr  id='s_${st.count }'>
   ...
   <a style="color: red"> 提示：选中资料可见</a>
    <!--  
      ...
      <div class="xzzl">
      <form action="addpastPicturdate.mht" id="myform" method="post">
...
<tr>
  ...
  <p></p>
   ...
   <tr style="width: 100%">
    ...
    <td  style="color: red;font-size: 12px" >三好资本是一个注重诚信的网络平台。如果我们发现您上传的资料系伪造或有人工修改痕迹，三好资本会将你加入系统黑名单，永久取消您在三好资本的借款资格。</td>
  </tr>
  ...
  $("#bt_sub").click(function(){
   ...
   //获取本来在数据库的图片获取他们的id
    ...
    $("input[name=items]").each(function() { 
     ...
     $.post("addpastPicturdate.mht",parama,function(data){
      if(data==123){
      alert('申请认证成功');
...
$(function(){
  ...
  $("#bt_sub_1").click(function(){
    ...
    $.post("addpastPicturdate.mht",paramimg,function(data){
    ...
    var gv = 0 ;
     ...
     $("#sub").click(function() {  
        ...
        $("input[name=items]").each(function() {  
           ...
           $.post("updatevisiable.mht",param,function(data){
             alert("操作成功");
