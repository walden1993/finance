  ...
  <tr>
    ...
    <td>${userMsg.lastDate }</td>
        ...
        <p style="padding:0">会员到期：<span>${userMsg.vip }</span></p>
         ...
         <a href="userCridit.mht?id=${userMsg.id }" class="scbtn">信用报告</a>
        </div>
      </div>
      </div>
      </div>
...
<script>
     ...
     $.post("queryUserRecoreIndex.mht",param,function(data){
      $("#Dynamiclist").html(data);
       ...
       $('#focusonUser').click(function(){
	     ...
	     //
	     $.post('focusonUser.mht',param,function(data){
	     	if(data == "virtual"){
	   ...
	   var param = {};
	      ...
	      $.post('mailInit.mht',param,function(data){
	       if(data == "virtual"){
	     		window.location.href = "noPermission.mht";
	     	}else{
...
<script>
 $(function(){
    ...
    $("#frieds").click(function(){
      ...
      param["paramMap.id"] = $('#uid').val();
      $.post("userFrends.mht",param,function(data){
      $("#frendlist").html(data);
       });
    });
    ...
    var param = {};
    param["paramMap.id"] = $('#uid').val();
    $.post("myBorrowList.mht",param,function(data){
      $("#browlist").html(data);
       });     
    }); 
    ...
    var param = {};
    param["paramMap.id"] = $('#uid').val();
    $.post("queryBorrowRecode.mht",param,function(data){
    ...
    var param = {};
    param["paramMap.id"] = $('#uid').val();
    $.post("queryfrendrecoredIndex.mht",param,function(data){
