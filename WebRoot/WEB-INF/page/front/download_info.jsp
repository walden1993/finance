...
<div class="tabmain">
  ...
  <div class="lcmain">
    ...
    <a href="downloads.mht?id=${paramMap.id}" class="chaxun" style="text-indent:0em;">文件下载</a>
    </p>
    </div>
    <div class="sxnews">
    <ul>
    <li style="background:none;line-height:20px">
      <s:if test="#request.previousDate!=null" >
         上一条：<a style="max-width: 30px;overflow: hidden;" href="frontDownloadDetail.mht?id=${request.previousDate.id}">${request.previousDate.title}</a>
      </s:if>  
      <s:else>
         <a>&nbsp;&nbsp;&nbsp;</a>
      </s:else>
    ...
    <li  style="background:none; line-height:20px">
     <s:if test="#request.lastDate!=null" >
         下一条：<a style="max-width: 30px;overflow: hidden;"   href="frontDownloadDetail.mht?id=${request.lastDate.id}">${request.lastDate.title}</a>
      </s:if> 
      <s:else>
        <a>&nbsp;&nbsp;&nbsp;</a>
      </s:else>
  ...
  dqzt(0)
	 ...
	 initNewsListInfo(null);
      ...
      function initNewsListInfo(praData){		
			$.dispatchPost("frontQueryNewsList.mht",praData,function(data){
