...
<!-- Baidu Button END -->
        ...
        <s:else>
	         <s:if test="%{#session.user==null}">
		        <s:if test='%{paramMap.debtStatus==2&&paramMap.remainDays!="过期"}'>
		        	<input type="button" class="jjiabtn" value="" onclick="location.href='auctingDebtInit.mht?debtId=${paramMap.debtId }'"/>
		        </s:if>
  ...
  <tr>
  <s:if test="#request.userMap.address==null||#request.userMap.address==''">
	    <td  align="right" >企业地址：
	    </td>
	    <td class="user_addressed">${userOrgMap.address} </td>
    </s:if>
    <s:else>
...
$(function(){
    ...
    //样式选中
	 ...
	 $.dispatchPost("queryDebtBorrowDetail.mht",param,function(data){
