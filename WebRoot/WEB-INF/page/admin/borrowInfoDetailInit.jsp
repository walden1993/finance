...
$(function(){
	...
	$("#excel").click(function(){
	 window.location.href="exportBorrowInfoDeatil.mht?pageBean.pageNum=1000 &&paramMap.borrowTitle="+$("#borrowTitle").val()+"&&paramMap.timeStart="+$("#timeStart").val()+"&&paramMap.timeEnd="+$("#timeEnd").val()+"&&paramMap.createTimeStart="+$("#createTimeStart").val()+"&&paramMap.createTimeEnd="+$("#createTimeEnd").val()+"&&paramMap.username="+$("#username").val()+"&&paramMap.borrowWay="+$("#borrowWay").val();
	});
});
function initListInfo(praData){
	...
	if ($("#timeStart").val().length > 9 && $("#timeEnd").val().length > 9){
		if ($("#timeStart").val() > $("#timeEnd").val()){
			alert("复审开始时间不能大于结束时间");
			return;
		}
	}
	if ($("#createTimeStart").val().length > 9 && $("#createTimeEnd").val().length > 9){
		...
		$.dispatchPost("borrowInfoDeatil.mht",praData,initCallBack);
	}
