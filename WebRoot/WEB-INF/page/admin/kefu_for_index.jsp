...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function initListInfo(praData){
			    praData["paramMap.kefuid"]='${paramMap.id}'	
		 		$.dispatchPost("queryKefuForInfo.mht",praData,initCallBack);
		 	}
			...
			function kefufors(data){
	 			...
	 			//window.location.href= "deleteKefu.mht?id="+ids;
	 			$.dispatchPost("addUserForkefubyId.mht",paramMap,function(data){
	           ...
	           if(callBack == undefined || callBack == ''){
	 				...
	 				if(data=='1'){
		 			  	alert("分配成功");
		 			  window.location.href="queryKefuForInit.mht?id="+'${paramMap.id}';
		 			}else{
	             		alert("分配失败");
		 			}
	 			});
		 	...
		 	function kefufor(id){
	 			...
	 			//window.location.href= "deleteKefu.mht?id="+id;
	 			$.dispatchPost("queryKefuForInfo.mht","",function(data){
	  ...
	  function sure(data){
		   ...
		   $.post("addUserForkefu.mht",param,function(data){
		        var callBack = data.msg;
	      if(callBack == undefined || callBack == ''){
		                 $('#right').html(data);
		                 $(this).show();
