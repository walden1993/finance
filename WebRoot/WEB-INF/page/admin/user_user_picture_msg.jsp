...
<script>
 ...
 var flag = true;
  ...
  $.post("updateallcheck.mht",param,function(data){
  var callBack = data.msg;
  if(data==1&&flag==false){
      $("#s_tip").html("审核成功^_^");
      window.location.href='queryPerUserPicturMsginitindex.mht?userId=${userId}&materAuthTypeId=${materAuthTypeIdStr}&userType=${userType}';
       flag = true;
  }else if(data==0){
   $("#s_tip").html("审核失败");
     flag = true;
  ...
  <tr>
     <div align="center">
        <table style="width: 100%; color: #333333;"
		  ...
		  <td >证件种类</td><td>认证图片</td><td >跟踪审核</td><td >认证审核</td><td>审核观点</td><td>上传时间</td><td >操作</td>
          </tr>
        	<s:if test="pageBean.page==null">
					<tr align="center" class="gvItem">
						<td>1</td><td>${username}</td>
         ...
         <s:iterator value="pageBean.page" var="bean" status="sta">
          <tr align="center" class="gvItem">                                                                                         
          <td>${sta.count }</td><td>${username}</td>
          <td>
			<s:if test="#bean.userType==1">
				<s:if test="#bean.realName==null">--</s:if>
		    <s:else>${bean.realName }/--</s:else>
		    </s:if>
			<s:if test="#bean.userType==2">
				<s:if test="#bean.orgName==null">--</s:if>
				<s:else>--/${bean.orgName }</s:else>
			</s:if>				
		 </td>
          <td>${tmtname }</td><td width="80px" height="80px">
          ...
          <a href="querybasepicture.mht?userId=${id }&TypeId=${materAuthTypeId}&adf=${tmdid }">审核</a>
          </s:if>
          <s:else><a href="querybasepicture.mht?userId=${id }&TypeId=${materAuthTypeId}&adf=${tmdid }">查看</a></s:else>
          </td>
             </tr>
          </s:iterator>
          </s:else>
