<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
<%@ include  file="/include/taglib.jsp"%>
<html>
	<head>
		<title>体验标投资记录</title> 
		<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
		<meta http-equiv="pragma"  content="no-cache" />
		<meta http-equiv="cache-control" content="no-cache" /> 
		<meta http-equiv="expires" content="0" />
		<meta http-equiv="keywords"  content="keyword1,keyword2,keyword3" />
		<meta http-equiv="description" content="This is my page" />
		<link href="../css/admin/admin_css.css" rel="stylesheet" type="text/css" />
		<link href="../css/admin/admin_custom_css.css" rel="stylesheet" type="text/css" />
	 </head>
	<body>
		<div id="right">
			<div style="padding: 15px 10px 0px 10px;">
				<div>
					<table border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td width="100" id="today"  class="xxk_all_a">
								<a href="queryExperienceInit.mht">体验标列表</a>
							</td>
							<td width="2">
								&nbsp;
							</td>
                            <td width="100" id="today"  class="main_alll_h2">
                                <a href="experienceBorrowInit.mht?id=${id}">体验记录</a>
                            </td>
                            <td width="2">
                                &nbsp;
                            </td>
                          </tr>
					</table>
				</div>
				<div  style="padding-right: 10px; padding-left: 10px; padding-bottom: 10px; width: 1160px; padding-top: 10px; background-color: #fff;">
					<table style="margin-bottom: 8px;" cellspacing="0" cellpadding="0"
						width="100%" border="0">
						<tbody>
						    <tr>
								<td class="f66" align="left" width="50%" height="36px">
									用户名：<input id="username"  value=""/>&nbsp;&nbsp;
									券号：<input id="ticketNo"  value=""/>&nbsp;&nbsp;
								    批次号：<input id="batchId"  value=""/>&nbsp;&nbsp;
								    <input id="search" type="button" value="&nbsp;&nbsp;查 询&nbsp;&nbsp;" name="search" />
								    <input id="excel" type="button" value="导出Excel" name="excel">
								</td>
							</tr>
						</tbody>
					</table>
		             <span id="divList"><img src="../images/admin/load.gif" class="load" alt="加载中..." /></span>
					<div>
				</div>
			</div>
			</div> 
		</div>
	</body>
<script type="text/javascript"  src="../script/jquery-2.0.js"></script>
<script type="text/javascript" src="../script/jquery.dispatch.js"></script>
<script  type="text/javascript" src="../css/admin/popom.js"></script>
<script  language="javascript" type="text/javascript" src="../My97DatePicker/WdatePicker.js"></script>
<script  type="text/javascript">
			$(function(){
				initListInfo(param);
				$('#search').click(function(){
					param["pageBean.pageNum"] = 1;
					param["paramMap.username"] = $("#username").val();
					param["paramMap.ticketNo"] = $("#ticketNo").val();
					param["paramMap.batchId"] = $("#batchId").val();
				   initListInfo(param);
				});
				$("#excel").click(function(){
	                   window.location.href=encodeURI(encodeURI("exportExperienceBorrow.mht?paramMap.username="+$("#username").val()+"&&paramMap.id="+${id}+"&&paramMap.ticketNo="+$("#ticketNo").val()+"&&paramMap.batchId="+$("#batchId").val()));
	             });
			});		
			function initListInfo(praData){
				param["paramMap.id"] = '${id}';
		 		$.post("experienceBorrowInfo.mht",praData,initCallBack);
		 	}
		 	function initCallBack(data){
				$("#divList").html(data);
			}
</script>
</html>
