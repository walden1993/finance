			...
			$(function(){
				...
				$("#btn_save").attr("disabled",true);
					...
					$("#btn_save").attr("disabled",false);
	 				...
	 				$.dispatchPost("addRole.mht",param,function(data){
	 					if(data.msg==1){
	 						alert("添加成功！");
	 						window.location.href = 'queryRoleList.mht';
	 						return ;
	 					}
	 					alert(data.msg);
	 				})
						...
						<table border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td height="28"  class="xxk_all_a">
									<a href="queryRoleList.mht">管理组列表</a>
								</td>
								<td width="2">
									&nbsp;
								</td>
								<td width="100" class="main_alll_h2">
									<a href="addRoleInit.mht">添加管理组</a>
								</td>
								<td width="2">
									&nbsp;
								</td>
								<td width="100" height="28"  class="xxk_all_a">
									<a href="queryAdminInit.mht">管理员列表</a>
								</td>
								<td width="2">
									&nbsp;
								</td>
								<td width="100"  class="xxk_all_a">
									<a href="addAdminInit.mht">添加管理员</a>
								</td>
								<td>
									&nbsp;
								</td>
								<td width="100"  class="xxk_all_a">
                                    <a href="addRights.mht">添加功能</a>
                                </td>
                                <td>
                                    &nbsp;
                                </td>
								...
								<tr>
									...
									<td class="f66">
										<s:iterator value="rightsList.{?#this.id<0}" var="bean">
											<input class="top" name="rightsId" id="top_${bean.id}"
												type="checkbox" value="${bean.id}" />${bean.summary }<br />
											<s:iterator value="rightsList.{?#this.parentID==#bean.id}"
												var="sBean">
												&nbsp;&nbsp;&nbsp;<input class="top_${bean.id} parent"
															name="rightsId" id="parent_${sBean.id }"
															parentId="${sBean.parentId }" type="checkbox"
															value="${sBean.id}" />
													${sBean.summary }
												<s:iterator value="rightsList.{?#this.parentID==#sBean.id}" var="sbBean">&nbsp;&nbsp;&nbsp;
												<input class="top_${bean.id} bottom parent_${sBean.id}"
																name="rightsId" parentId="${sbBean.parentId }"
																type="checkbox" value="${sbBean.id}" />
													${sbBean.summary}
												</s:iterator>
												<br />
											</s:iterator>
