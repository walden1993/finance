...
<script>
 ...
 var flag = true;
  ...
  if(data==1){
      $("#s_tip").html("审核成功");
          window.location.href="queryselectindex.mht?userId=${userId}&userType=${userType}&TypeId=${materAuthTypeIdStr}";
       flag = true;
  }else if(data==0){
   $("#s_tip").html("审核失败");
     flag = true;
  ...
  <tr>
     <div align="center">
        ...
        <s:iterator value="pageBean.page" var="bean" status="sta">
          ...
          <s:elseif test="#bean.userType==2">
          <th >企业全称</th>
          </s:elseif>
          <s:else>
          <th >不存在</th>
          </s:else>
          <th >证件种类</th><th>认证图片</th><th >跟踪审核</th><th >认证审核</th><th>审核观点</th><th>上传时间</th><th >操作</th>
          </tr>
        </s:iterator>
         ...
         <s:iterator value="pageBean.page" var="bean" status="sta">
          ...
          <a href="queryPersonPictureDateindex.mht?userId=${id }&userType=${userType}&TypeId=${materAuthTypeId}&adf=${tmdid }">审核</a>
          </s:if>
          <s:else><a href="queryPersonPictureDateindex.mht?userId=${id }&userType=${userType}&TypeId=${materAuthTypeId}&adf=${tmdid }">查看</a></s:else>
          </td>
             </tr>
          </s:iterator>
          </s:else>
