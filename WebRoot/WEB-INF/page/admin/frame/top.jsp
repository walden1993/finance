...
<html>
	...
	function loginOut() {
		if (confirm("是否确定要退出？")) {
			window.parent.location = "adminLoginOut.mht";
		}
	}
	$(function(){
			...
			<div class="daohang_all"  >
						...
						<s:iterator value="#session.adminRoleMenuList.{?#this.rightsId<0}"
							...
							<s:if test="#bean.rightsId == -11">
								 <!--<li>
									<a href="logging.mht?action=toLogin&admin=admin" target="_blank">${bean.summary }</a> 
                                </li> -->
                                
							</s:if>
							<s:else>
								<li>
									<a href="showsubmenu.mht?index=${bean.rightsId }" target="left">${bean.summary }</a>
								</li>
							</s:else>
						</s:iterator>
                    	</ul>
